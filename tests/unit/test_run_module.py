...
- Two skip-before-task paths (circuit-breaker, lcma-tools-unavailable)
  via the scheduler-prelude integration
...
async def test_run_execute_propagates_sweep_write_error_with_health_action() -> None:
    ...
    assert probe_loop.marked is True, (
        "RunAborted's diagnostics should have flipped the repair latch"
    )
    assert probe_loop.cleared is False
    # Lithos task closed with outcome=error.
...
async def test_run_execute_skips_repair_for_backfill() -> None:
    ...
    # Backfill tag on the task_create call.
...
@pytest.mark.asyncio
async def test_run_aborted_carries_diagnostics_for_apply() -> None:
