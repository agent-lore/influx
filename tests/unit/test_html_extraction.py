"""Tests for HTML extraction (US-006).

Covers: success >= min_html_chars, rejection < min_html_chars,
tag-stripping for each default-stripped tag, no HTML fragments in output,
and HTTP/extractor failure propagation.
"""

from __future__ import annotations

from pathlib import Path
from unittest.mock import patch

import pytest

from influx.errors import ExtractionError, NetworkError
from influx.extraction.html import ExtractionResult, _strip_tags, extract_html
from influx.http_client import FetchResult

_FIXTURES = Path(__file__).resolve().parent.parent / "fixtures" / "extraction"


def _read_fixture(name: str) -> bytes:
    return (_FIXTURES / name).read_bytes()


def _make_fetch_result(body: bytes) -> FetchResult:
    return FetchResult(
        body=body,
        status_code=200,
        content_type="text/html; charset=utf-8",
        final_url="https://arxiv.org/html/2601.12345",
    )


# ── Success path ────────────────────────────────────────────────────


class TestSuccessPath:
    """Extracted text >= min_html_chars returns ExtractionResult."""

    @patch("influx.extraction.html.guarded_fetch")
    def test_returns_extraction_result_for_good_article(
        self, mock_fetch: object
    ) -> None:
        html = _read_fixture("good_article.html")
        mock_fetch.return_value = _make_fetch_result(html)  # type: ignore[union-attr]

        result = extract_html(
            "https://arxiv.org/html/2601.12345",
            min_html_chars=100,
        )

        assert isinstance(result, ExtractionResult)
        assert result.source == "html"
        assert len(result.text) >= 100

    @patch("influx.extraction.html.guarded_fetch")
    def test_extracted_text_has_no_html_tags(self, mock_fetch: object) -> None:
        html = _read_fixture("good_article.html")
        mock_fetch.return_value = _make_fetch_result(html)  # type: ignore[union-attr]

        result = extract_html(
            "https://arxiv.org/html/2601.12345",
            min_html_chars=100,
        )

        assert "<" not in result.text
        assert ">" not in result.text

    @patch("influx.extraction.html.guarded_fetch")
    def test_passes_guard_params_to_fetch(self, mock_fetch: object) -> None:
        html = _read_fixture("good_article.html")
        mock_fetch.return_value = _make_fetch_result(html)  # type: ignore[union-attr]

        extract_html(
            "https://arxiv.org/html/2601.12345",
            min_html_chars=100,
            allow_private_ips=True,
            max_download_bytes=1000000,
            timeout_seconds=10,
        )

        mock_fetch.assert_called_once_with(  # type: ignore[union-attr]
            "https://arxiv.org/html/2601.12345",
            allow_private_ips=True,
            max_download_bytes=1000000,
            timeout_seconds=10,
            expected_content_type="html",
        )


# ── Rejection (below min_html_chars) ────────────────────────────────


class TestMinLengthRejection:
    """Extracted text < min_html_chars raises ExtractionError."""

    @patch("influx.extraction.html.guarded_fetch")
    def test_rejects_short_article_below_default_threshold(
        self, mock_fetch: object
    ) -> None:
        html = _read_fixture("short_article.html")
        mock_fetch.return_value = _make_fetch_result(html)  # type: ignore[union-attr]

        with pytest.raises(ExtractionError, match="too short"):
            extract_html(
                "https://arxiv.org/html/2601.12345",
                min_html_chars=1000,
            )

    @patch("influx.extraction.html.guarded_fetch")
    def test_800_char_fixture_rejected_at_1000_threshold(
        self, mock_fetch: object
    ) -> None:
        """AC-07-E: 800-char extraction fails at min_html_chars=1000."""
        html = _read_fixture("short_article.html")
        mock_fetch.return_value = _make_fetch_result(html)  # type: ignore[union-attr]

        with pytest.raises(ExtractionError, match="too short"):
            extract_html(
                "https://arxiv.org/html/2601.12345",
                min_html_chars=1000,
            )


# ── Tag-stripping ────────────────────────────────────────────────────


class TestTagStripping:
    """Tags in extraction.strip_tags are removed before extraction."""

    @patch("influx.extraction.html.guarded_fetch")
    def test_script_tag_stripped(self, mock_fetch: object) -> None:
        html = _read_fixture("with_script.html")
        mock_fetch.return_value = _make_fetch_result(html)  # type: ignore[union-attr]

        result = extract_html(
            "https://arxiv.org/html/2601.12345",
            min_html_chars=10,
        )

        assert "malicious" not in result.text
        assert "alert" not in result.text
        assert "injected content" not in result.text.lower()

    @patch("influx.extraction.html.guarded_fetch")
    def test_iframe_tag_stripped(self, mock_fetch: object) -> None:
        html = _read_fixture("with_iframe.html")
        mock_fetch.return_value = _make_fetch_result(html)  # type: ignore[union-attr]

        result = extract_html(
            "https://arxiv.org/html/2601.12345",
            min_html_chars=10,
        )

        assert "evil.example.com" not in result.text
        assert "iframe content" not in result.text.lower()

    @patch("influx.extraction.html.guarded_fetch")
    def test_object_tag_stripped(self, mock_fetch: object) -> None:
        html = _read_fixture("with_object.html")
        mock_fetch.return_value = _make_fetch_result(html)  # type: ignore[union-attr]

        result = extract_html(
            "https://arxiv.org/html/2601.12345",
            min_html_chars=10,
        )

        assert "malware" not in result.text
        assert "object content" not in result.text.lower()

    @patch("influx.extraction.html.guarded_fetch")
    def test_embed_tag_stripped(self, mock_fetch: object) -> None:
        html = _read_fixture("with_embed.html")
        mock_fetch.return_value = _make_fetch_result(html)  # type: ignore[union-attr]

        result = extract_html(
            "https://arxiv.org/html/2601.12345",
            min_html_chars=10,
        )

        assert "plugin.swf" not in result.text

    def test_strip_tags_removes_script_content(self) -> None:
        html = "<p>before</p><script>evil();</script><p>after</p>"
        result = _strip_tags(html, ["script"])
        assert "evil" not in result
        assert "before" in result
        assert "after" in result

    def test_strip_tags_removes_self_closing(self) -> None:
        html = '<p>text</p><embed src="x.swf"/><p>more</p>'
        result = _strip_tags(html, ["embed"])
        assert "x.swf" not in result
        assert "text" in result
        assert "more" in result


# ── No HTML fragments in output ──────────────────────────────────────


class TestNoHtmlFragments:
    """Output contains no HTML fragments — clean text only (FR-RES-5)."""

    @patch("influx.extraction.html.guarded_fetch")
    def test_output_is_clean_text(self, mock_fetch: object) -> None:
        html = _read_fixture("good_article.html")
        mock_fetch.return_value = _make_fetch_result(html)  # type: ignore[union-attr]

        result = extract_html(
            "https://arxiv.org/html/2601.12345",
            min_html_chars=100,
        )

        # No HTML angle brackets in clean text
        assert "<" not in result.text
        assert ">" not in result.text
        # No common HTML entities
        assert "&lt;" not in result.text
        assert "&gt;" not in result.text
        assert "&amp;" not in result.text


# ── Failure propagation ──────────────────────────────────────────────


class TestFailurePropagation:
    """HTTP failure or extractor exception surfaces to caller."""

    @patch("influx.extraction.html.guarded_fetch")
    def test_network_error_propagates(self, mock_fetch: object) -> None:
        mock_fetch.side_effect = NetworkError(  # type: ignore[union-attr]
            "Connection refused",
            url="https://arxiv.org/html/2601.12345",
            kind="network",
            reason="refused",
        )

        with pytest.raises(NetworkError):
            extract_html("https://arxiv.org/html/2601.12345")

    @patch("influx.extraction.html.guarded_fetch")
    def test_trafilatura_returns_none_raises_extraction_error(
        self, mock_fetch: object
    ) -> None:
        # Minimal HTML that trafilatura can't extract content from
        mock_fetch.return_value = _make_fetch_result(  # type: ignore[union-attr]
            b"<html><body></body></html>"
        )

        with pytest.raises(ExtractionError, match="no content"):
            extract_html("https://arxiv.org/html/2601.12345")
