...
Over-length lists are now **truncated to the cap on ingest**, keeping the leading items — the prompt asks for them "prioritised by relevance", so the tail is the cheapest part to drop. Truncation is logged at INFO with the field name and the number of items dropped, so the loss stays observable. This follows the precedent already set for element *content*, which has always been truncated to 500 characters rather than rejected.
